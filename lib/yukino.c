...
/* helper function -- iterates over everything and puts it into a dynamically
 * allocated list the reason this does not call _start() is so that programs can
 * still benefit from the asynchronous calls that XCB provides */
	...
	yukino_window_iter_t *wi, yukino_window_t **pwin, size_t *pwinsz)
{
	...
	while ((r = yukino_window_iter(conn, wi, &newwin)) >= 0) {
		...
		if (winlistsz >= winlistalloc) {
			...
			winlist = realloc(
				winlist, winlistalloc * sizeof(*winlist));
...
static yukino_result_t take_cb(void *conn, uint32_t x, uint32_t y, uint32_t w,
	uint32_t h, yukino_take_pixel pixel_func, void *userdata)
{
